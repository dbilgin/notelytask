...
- After publishing a GitHub release, `upload-downloads` replaces only `/notelytask` in SFTPGo's `ci-builds` namespace through `https://download.dbilgin.com/api/v2/user` with the APK, DEB, and RPM. It uses secret `SFTPGO_API_KEY` bound to that user, with API-key authentication enabled. No SSH configuration is required. Validate all local packages and authenticate before deleting old files; reject a symlink destination and never clear another directory. Verify uploaded filenames and sizes. Uploads are serialized. Delete-first storage limits mean a failed upload can leave incomplete downloads; rerun the job to recover. GitHub releases contain generated notes only, without uploaded packages or package links; Actions artifacts remain the build-to-upload handoff.
- Tagged releases also build Flutter web using GitHub's `ENV_FILE` secret, publish `ghcr.io/dbilgin/notelytask-web`, and roll out the immutable image digest to Dokploy after the native release succeeds. Dokploy does not access GitHub source. Its runtime environment does not configure compiled Flutter web.
- Dokploy rollout uses GitHub secret `DOKPLOY_API_KEY` and optionally `GHCR_READ_TOKEN` (persistent read:packages access for private images). Without registry credentials, the GHCR package must be public. `deploy/nginx.conf` serves SPA routes on port 8080; the deployment script verifies the released commit via `/release.txt`. Firebase remains manual.
