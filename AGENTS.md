...
- `create-release` uploads APK/DEB/RPM to GitHub Releases and runs `deploy/retain_release.py` with `GITHUB_TOKEN` (`contents: write`). Verify all three package names, uploaded states, and sizes before deleting only known NotelyTask package assets from lower stable semantic versions. Preserve tags, notes, unrelated assets, drafts, prereleases, and newer versions. Paginate releases/assets; serialize publication and cleanup. Verification failure must prevent deletion. SFTPGo uploads are retired. Tests: `python3 -m unittest discover -s deploy -p 'test_*.py'`.
